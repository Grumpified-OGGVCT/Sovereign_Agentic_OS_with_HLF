...
import agents.gateway.matrix_sync.scheduler as _sched
from agents.gateway.matrix_sync.pipeline import run_pipeline_scheduled
from agents.gateway.matrix_sync.scheduler import (
...
def _reset_scheduler_state() -> None:
    ...
    _sched._last_run_time = None
    _sched._last_run_status = "never_run"
    _sched._last_run_error = ""
    _sched._next_run_time = None
...
class TestStartStopScheduler:
    ...
    def test_start_disabled_does_not_create_scheduler(self, tmp_path: Path) -> None:
        ...
        with patch.dict("os.environ", {"BASE_DIR": str(tmp_path)}):
            start_scheduler()
            assert _sched._scheduler is None
    ...
    def test_double_start_is_safe(self, tmp_path: Path) -> None:
        ...
        with patch.dict("os.environ", {"BASE_DIR": str(tmp_path)}):
            ...
            assert _sched._scheduler is not None and _sched._scheduler.running
...
class TestRunJob:
    ...
    def test_run_job_on_success_sets_ok_status(self) -> None:
        with (
            ...
            _run_job()
        assert _sched._last_run_status == "ok"
        assert _sched._last_run_error == ""
        assert _sched._last_run_time is not None
    ...
    def test_run_job_on_error_sets_error_status(self) -> None:
        ...
        assert "network down" in _sched._last_run_error
