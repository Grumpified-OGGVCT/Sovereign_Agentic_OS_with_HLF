...
> Created: March 10, 2026 — Sovereign Agentic OS Session
...
External apps are **not** plugins, adapters, or browser embeddings. They are HLF host functions registered in the `HostFunctionRegistry`, dispatched through `host_function_dispatcher.py`, and governed by the full LALR(1) → 6-gate → bytecode VM → Scribe/Sentinel audit pipeline.
...
| App | HLF Host Functions | API Endpoint |
|-----|-------------------|-------------|
...
- [ ] Wire API-Keeper as credential vault persistence for `model_gateway.py`
- [ ] Wire SearXng_MCP as `WEB_SEARCH` backend in `host_function_dispatcher.py`
...
- [ ] MoE routing merge from moe-ollama-endpoint into `model_gateway.py`
