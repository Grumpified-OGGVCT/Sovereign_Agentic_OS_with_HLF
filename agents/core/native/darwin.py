"""
DarwinBridge — Enterprise macOS native integration.

Extends NativeBridge with:
  - macOS-specific commands (osascript, defaults, open, etc.)
  - ALSLogger audit trail
  - Config-driven feature flags
  - rumps/pystray tray detection
"""
...
_logger = ALSLogger(agent_role="native-darwin", goal_id="bridge")


class DarwinBridge(NativeBridge):
    """macOS-specific NativeBridge implementation."""

    _PLATFORM_COMMANDS: frozenset[str] = frozenset({
        "open", "osascript", "defaults", "launchctl",
        "pbcopy", "pbpaste", "sw_vers", "system_profiler",
        "diskutil", "networksetup", "scutil", "security",
        "mdutil", "mdfind", "sips", "say",
        "caffeinate", "pmset", "softwareupdate",
        "xcode-select", "instruments",
    })
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        _logger.log("BRIDGE_INIT", {"platform": "darwin", "features": self._features})
            ...
            try:
                ...
                for line in result.stdout.strip().split("\n")[1:51]:
                    ...
                    if len(parts) >= 11:
                        ...
                        try:
                            processes.append(ProcessInfo(
                                pid=int(parts[1]), name=name, status="running",
    ...
    def shell_exec(
        ...
        timeout_seconds: float = 30.0, cwd: str | None = None,
    ) -> ShellResult:
        ...
        return _exec(command, args, timeout_seconds=timeout, cwd=cwd, bridge=self,
