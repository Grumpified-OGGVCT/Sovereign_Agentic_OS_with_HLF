"""
LinuxBridge — Enterprise Linux native integration.

Extends NativeBridge with:
  - Linux-specific commands (xdg-open, systemctl, journalctl, etc.)
  - ALSLogger audit trail
  - Config-driven feature flags
  - /proc fallback for process listing
"""
...
_logger = ALSLogger(agent_role="native-linux", goal_id="bridge")


class LinuxBridge(NativeBridge):
    """Linux-specific NativeBridge implementation."""
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        _logger.log("BRIDGE_INIT", {"platform": "linux", "features": self._features})
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    ["ps", "aux", "--no-headers"], capture_output=True, text=True, timeout=10,
                )
