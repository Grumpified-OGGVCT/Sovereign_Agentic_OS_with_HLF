...
def build_diff(previous_rows: list[dict[str, str]], current_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for k in all_keys:
        ...
        if p and not c:
            out.append({
                "category": k[0], "official_nomenclature": k[1], "change_type": "REMOVED",
                "prev_tier": p.get("tier", ""), "curr_tier": "",
                "prev_score": p.get("category_score", ""), "curr_score": "",
                "prev_confidence": p.get("confidence_score", ""), "curr_confidence": "",
                "tier_delta": "", "score_delta": "", "confidence_delta": "",
            })
            continue

        if c and not p:
            out.append({
                "category": k[0], "official_nomenclature": k[1], "change_type": "NEW_ENTRY",
                "prev_tier": "", "curr_tier": c.get("tier", ""),
                "prev_score": "", "curr_score": c.get("category_score", ""),
                "prev_confidence": "", "curr_confidence": c.get("confidence_score", ""),
                "tier_delta": "", "score_delta": "", "confidence_delta": "",
            })
            continue
        ...
        out.append({
            "category": k[0], "official_nomenclature": k[1], "change_type": change,
            "prev_tier": pt, "curr_tier": ct,
            "prev_score": p.get("category_score", ""), "curr_score": c.get("category_score", ""),
            "prev_confidence": p.get("confidence_score", ""), "curr_confidence": c.get("confidence_score", ""),
