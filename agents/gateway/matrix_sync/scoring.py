...
def category_scores(model_name: str, card: CardInfo, rank_info: Optional[Dict[str, Any]]) -> Dict[str, float]:
    ...
    for kw in ["coding","code","swe","terminal","repo","multi-file","developer","software engineering"]:
        if kw in txt: s["Programming & Code Engineering"] += 1.2
    if "tools" in caps: s["Programming & Code Engineering"] += 1.0
    s["Programming & Code Engineering"] += comp * 5.0

    for kw in ["reasoning","thinking","aime","gpqa","math","long-horizon","planning"]:
        if kw in txt: s["Reasoning & Long-Horizon Planning"] += 1.0
    if "thinking" in caps: s["Reasoning & Long-Horizon Planning"] += 1.2
    s["Reasoning & Long-Horizon Planning"] += comp * 4.5

    for kw in ["vision","vl","ocr","image","video","gui","multimodal","document"]:
        if kw in txt: s["Vision & Multimodal Intelligence"] += 1.0
    if "vision" in caps: s["Vision & Multimodal Intelligence"] += 1.3
    ...
    for kw in ["open-weight","mit license","commercial use","developer use","open source"]:
        if kw in txt: s["Open-Weight Developer Stack"] += 0.9
    ...
    if rank_info:
        ...
        if r <= 3: s["Generalist Frontier Assistant"] += 2.0
        elif r <= 10: s["Generalist Frontier Assistant"] += 1.0
